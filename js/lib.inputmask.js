...
(function(factory) {
    ...
    Inputmask.prototype = {
        ...
        defaults: {
            ...
            //executes on every key-press with the result of isValid. Params: result, opts
            ...
            supportsInputType: [],
            //allow extra inputtypes for masking, ex. number can be allowed for numeric alias without pre-/suffix and standard radixpoint, groupSeparator
            definitions: {
                '9': {
                    validator: '[0-9]',
                    cardinality: 1,
    ...
    function generateMaskSet(opts, nocache) {
        ...
        function analyseMask(mask) {
            var tokenizer = /(?:[?*+]|\{[0-9\+\*]+(?:,[0-9\+\*]*)?\})|[^.?*+^${[]()|\\]+|./g, escaped = false, currentToken = new MaskToken(), match, m, openenings = [], maskTokens = [], openingToken, currentOpeningToken, alternator, lastMatch, groupToken;
                ...
                if (openenings.length > 0) {
                    ...
                    if (currentOpeningToken.isAlternator) {
                        ...
                        for (var mndx = 0; mndx < alternator.matches.length; mndx++) {
                            ...
                            //don't mark alternate groups as group
                        }
                        if (openenings.length > 0) {
                            currentOpeningToken = openenings[openenings.length - 1];
    ...
    function maskScope(actionObj, maskset, opts) {
        var isRTL = false, undoValue, // compositionValidPos,
        compositionCaretPos, compositionData, el, $el, skipKeyPressEvent = false, //Safari 5.1.x - modal dialog fires keypress twice workaround
        skipInputEvent = false, //skip when triggered from within inputmask
        ignorable = false, maxLength, mouseEnter = true;
        ...
        function getMaskTemplate(baseOnInput, minimalPos, includeInput) {
            ...
            do {
                ...
                pos++;
            } while ((maxLength === undefined || pos - 1 < maxLength) && test.fn !== null || (test.fn === null && test.def !== '') || minimalPos >= pos);maskTemplate.pop();
            //drop the last one which is empty
            return maskTemplate;
        }
        ...
        function getTests(pos, ndxIntlzr, tstPs, cacheable) {
            var maskTokens = getMaskSet().maskToken, testPos = ndxIntlzr ? tstPs : 0, ndxInitializer = ndxIntlzr || [0], matches = [], insertStop = false, latestMatch, isFirstMatch;

            function resolveTestFromToken(maskToken, ndxInitializer, loopNdx, quantifierRecurse) {
                //ndxInitilizer contains a set of indexes to speedup searches in the mtokens
                    ...
                    if (testPos === pos && match.matches === undefined) {
                        ...
                        if (match.isGroup && quantifierRecurse !== match) {
                            ...
                            if (altIndex === -1 || typeof altIndex === 'string') {
                                ...
                                //insert a stopelemnt when there is an alternate
                            } else {
                                if (alternateToken.matches[altIndex]) {
                                    //if not in the initial alternation => look further
            ...
            //set a clone to prevent overwriting some props
        ...
        function isValid(pos, c, strict, fromSetValid) {
                    ...
                    if (rslt !== false) {
                        ...
                        if (rslt.refreshFromBuffer) {
                            ...
                            if (rslt.pos === undefined && rslt.c === undefined) {
                                ...
                                return false;
                                //breakout if refreshFromBuffer && nothing to insert
                            }
            ...
            function alternate(pos, c, strict, fromSetValid) {
                var validPsClone = $.extend(true, {}, getMaskSet().validPositions), lastAlt, alternation, isValidRslt, altPos, i, validPos;
                            ...
                            for (var mndx = 0; mndx < altNdxs.length; mndx++) {
                                if (decisionTaker < altNdxs[mndx]) {
                                    ...
                                    if (decisionTaker !== possibilityPos.locator[alternation]) {
                                        ...
                                        //disable keepStatic on getMaskLength
                                        isValidRslt = true;
        ...
        function isMask(pos) {
            ...
            if (test.fn != null) {
                ...
                for (var i = 0; i < tests.length; i++) {
                    if (tests[i].match.def !== '' && (/*tests[i].match.fn !== null || */
                    (tests[i].alternation === undefined || tests[i].locator[tests[i].alternation].length > 1))) {
        ...
        function determineLastRequiredPosition(returnDefinition) {
            var buffer = getBuffer(), bl = buffer.length, pos, lvp = getLastValidPosition(), positions = {}, lvTest = getMaskSet().validPositions[lvp], ndxIntlzr = lvTest !== undefined ? lvTest.locator.slice() : undefined, testPos;
        ...
        function handleRemove(input, k, pos, strict) {
            function generalize() {
                if (opts.keepStatic) {
                    resetMaskSet(true);
                    var validInputs = [], lastAlt, positionsClone = $.extend(true, {}, getMaskSet().validPositions);
                ...
                if (k) {
                    ...
                    if (valResult !== false) {
                        if (valResult !== true) {
                            ...
                            //set new position from isValid
                            c = valResult.c !== undefined ? valResult.c : c;
                            //set new char from isValid
                        }
                        resetMaskSet(true);
        ...
        function inputFallBackEvent(e) {
            //fallback when keypress & compositionevents fail
            var input = this;
        ...
        function compositionEndEvent(e) {//pickup by inputfallback
        }
            ...
            if ((el.tagName === 'INPUT' && isInputTypeSupported(el.getAttribute('type'))) || el.isContentEditable) {
                ...
                $(el.form).on('submit', function() {
                    ...
                    if (opts.clearMaskOnLostFocus && getLastValidPosition() === -1 && el.inputmask._valueGet && el.inputmask._valueGet() === getBufferTemplate().join('')) {
                        ...
                        //clear masktemplete on submit and still has focus
                    }
                ...
                if (!androidfirefox) {
                    $el.on('compositionstart.inputmask', wrapEvent(compositionStartEvent)).on('compositionupdate.inputmask', wrapEvent(compositionUpdateEvent)).on('compositionend.inputmask', wrapEvent(compositionEndEvent));
                }
            ...
            case 'unmaskedvalue':
                ...
                if (el === undefined) {
                    ...
                    if ($.isFunction(opts.onBeforeWrite))
                        opts.onBeforeWrite.call(this, undefined, getBuffer(), 0, opts);
                } else
                    $el = $(el);
                maskset = el.inputmask.maskset;
                opts = el.inputmask.opts;
